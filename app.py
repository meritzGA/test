...
def calculate_prize_for_code(target_code, prize_config, df_src):
    ...
    for cfg in prize_config:
        ...
        prize = _safe_float_prize(raw_prize)
    ...
    if has_data():
        ...
        if st.button("➕ 주차/브릿지 시상 추가", key="add_prize_weekly"):
            prize_cfgs.append({
                ...
                "col_val_prev": "", "col_val_curr": "", "col_prize": ""
            })
        ...
        for idx, cfg in weekly_prizes:
            with st.expander(f"📌 {cfg.get('name', '시책')} ({cfg.get('type', '구간')})", expanded=False):
                ...
                cfg['col_prize'] = st.selectbox("💰 시상금 열 (필수)", cols, index=_gi(cfg.get('col_prize',''), cols), key=f"pprize_{idx}")
                st.info("✅ 구간/지급률 계산 없이, 지정한 열에서 **실적**과 **시상금**을 직접 가져옵니다.")
