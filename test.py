...
def calculate_agent_performance(target_code):
    ...
    # 총 시상 = 누계 시상 + 브릿지 시상만 (구간 시책은 누계에 이미 포함되므로 제외)
    cumul_total = sum(r['prize'] for r in calculated_results if r['category'] == 'cumulative')
    bridge_total = sum(r['prize'] for r in calculated_results if r['category'] == 'weekly' and '브릿지' in r['type'])
    total_prize_sum = cumul_total + bridge_total
...
def render_ui_cards(user_name, calculated_results, total_prize_sum, show_share_text=False):
    ...
    # 구간 시책(진도)과 브릿지를 분리
    gugan_res = [r for r in weekly_res if r['type'] == '구간']
    bridge_res = [r for r in weekly_res if '브릿지' in r['type']]
    
    gugan_total = sum(r['prize'] for r in gugan_res)
    bridge_total = sum(r['prize'] for r in bridge_res)
    cumul_total = sum(r['prize'] for r in cumul_res)

    share_text = f"🎯 [{user_name} 팀장님 실적 현황]\n"
    share_text += f"💰 총 예상 시상금: {total_prize_sum:,.0f}원\n"
    share_text += f"   (누계 {cumul_total:,.0f}원 + 브릿지 {bridge_total:,.0f}원)\n"
    share_text += "────────────────\n"

    # ── 구간 시책 진행 현황 (진도 안내용, 총 시상에 미포함) ──
    if gugan_res:
        summary_html = (
            f"<div class='summary-card'>"
            f"<div class='summary-label'>{user_name} 팀장님의 시책 진행 현황</div>"
            f"<div class='summary-total'>{gugan_total:,.0f}원</div>"
            f"<div style='color:rgba(255,255,255,0.7); font-size:0.95rem; margin-top:-18px; margin-bottom:16px;'>※ 아래 누계 시상에 포함된 금액입니다</div>"
            f"<div class='summary-divider'></div>"
        )
        share_text += f"📌 [시책 진행 현황] (누계에 포함)\n"
        
        for res in gugan_res:
    ...
    if bridge_res:
        bridge_summary_html = (
            f"<div class='summary-card' style='background: linear-gradient(135deg, #ff6b35 0%, #d4380d 100%); box-shadow: 0 10px 25px rgba(212, 56, 13, 0.25);'>"
            f"<div class='summary-label'>{user_name} 팀장님의 브릿지 시상</div>"
            f"<div class='summary-total'>{bridge_total:,.0f}원</div>"
            f"<div class='summary-divider'></div>"
        )
        share_text += f"\n🌉 [브릿지 시상]\n"
        
        for res in bridge_res:
            if res['type'] == "브릿지1":
                bridge_summary_html += f"<div class='data-row' style='padding: 6px 0;'><span class='summary-item-name'>{res['name']}</span><span class='summary-item-val'>{res['prize']:,.0f}원</span></div>"
                share_text += f"🔹 {res['name']}: {res['prize']:,.0f}원\n"
            else:
                bridge_summary_html += f"<div class='data-row' style='padding: 6px 0; align-items:flex-start;'><span class='summary-item-name'>{res['name']}<br><span style='font-size:0.95rem; color:rgba(255,255,255,0.7);'>(다음 달 {int(res['curr_req']//10000)}만 가동 조건)</span></span><span class='summary-item-val'>{res['prize']:,.0f}원</span></div>"
                share_text += f"🔹 {res['name']}: {res['prize']:,.0f}원 (다음 달 {int(res['curr_req']//10000)}만 가동 조건)\n"
                
        bridge_summary_html += "</div>"
        st.markdown(bridge_summary_html, unsafe_allow_html=True)
    ...
    # ── 총 예상 시상금 합산 (누계 + 브릿지) ──
    if total_prize_sum > 0:
        total_html = (
            f"<div style='background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); "
            f"border-radius: 20px; padding: 28px 24px; margin-top: 20px; margin-bottom: 16px; "
            f"border: 2px solid #ffd700; box-shadow: 0 10px 25px rgba(0,0,0,0.3);'>"
            f"<div style='color: #ffd700; font-size: 1.15rem; font-weight: 600; margin-bottom: 8px;'>💰 총 예상 시상금</div>"
            f"<div style='color: #ffffff; font-size: 2.6rem; font-weight: 800; letter-spacing: -1px; margin-bottom: 16px;'>{total_prize_sum:,.0f}원</div>"
            f"<div style='height: 1px; background-color: rgba(255,215,0,0.3); margin: 12px 0;'></div>"
        )
        if cumul_total > 0:
            total_html += f"<div class='data-row' style='padding: 6px 0;'><span style='color: rgba(255,255,255,0.85); font-size: 1.1rem;'>누계 시상</span><span style='color: #ffffff; font-size: 1.2rem; font-weight: 700;'>{cumul_total:,.0f}원</span></div>"
        if bridge_total > 0:
            total_html += f"<div class='data-row' style='padding: 6px 0;'><span style='color: rgba(255,255,255,0.85); font-size: 1.1rem;'>브릿지 시상</span><span style='color: #ffffff; font-size: 1.2rem; font-weight: 700;'>{bridge_total:,.0f}원</span></div>"
        if gugan_total > 0:
            total_html += f"<div style='color: rgba(255,255,255,0.5); font-size: 0.9rem; margin-top: 10px;'>※ 시책 진행 {gugan_total:,.0f}원은 누계에 포함</div>"
        total_html += "</div>"
        st.markdown(total_html, unsafe_allow_html=True)
        
        share_text += f"\n━━━━━━━━━━━━━━━━\n"
        share_text += f"💰 총 예상 시상금: {total_prize_sum:,.0f}원\n"
        share_text += f"  (누계 {cumul_total:,.0f} + 브릿지 {bridge_total:,.0f})\n"
